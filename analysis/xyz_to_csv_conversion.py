...
def xyz_to_csv(
    ...
    xyz_filename="/config0.xyz",
    parameters_filename="/input_values.txt",
    debug=False,
):
    if debug:
        print("transforming xyz files")
    ...
    name_of_csv_file = (
        new_directory
        + "F"
        + str(parameter_dict["activity"])
        + "_K"
        + str(parameter_dict["bending"])
        + "_theta"
        + str((parameter_dict["ChiralityAngle"] * 180) / np.pi)
        + ".csv"
    )
